...
def gen_hist_hier(image, bins, hierarchy, num_dimensions):
    H, W, C = image.shape
    hists = []
    for level in range(hierarchy):
        blocks = 2 ** level
        windowed_image = divide_image(image, blocks)
        block_hists = []
        for image_window in windowed_image:
            ...
            block_hists.append(hist)
        block_hists = np.stack(block_hists, axis=0).ravel()
        hists.append(block_hists)
    hists = np.concatenate(hists, axis=0).ravel()
    return hists
...
if __name__ == '__main__':
    ...
    hist1 = gen_hist(image, bins, num_windows, num_dimensions)
    hist2 = gen_hist_hier(image, bins, hierarchy, num_dimensions)
    print('Yay!')