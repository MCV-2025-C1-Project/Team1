...
def parse_args():
    ...
    parser.add_argument('--hierarchy_list', nargs='+', type=int, default=[1],
                        help="Hierarchy levels of the histogram pyramid.")
...
def main():
    ...
    if val:
        ...
        grid_search_df = pd.DataFrame(columns=['color_space', 'preprocess', 'bins', 'hierarchy', 'hist_dim', 'distances', *[f'mapk{k}' for k in k_list]])
        
        with open(os.path.join(query_path, 'gt_corresps.pkl'), 'rb') as f:
            groundtruth = pickle.load(f)
    
    db = database.Database(db_path, color_space=color_space, preprocess=preprocess, hierarchy=True)
    ...
    num_tests = len(bins_list) * len(hierarchy_list) * len(hist_dims_list) * len(distances_list)
    with tqdm(total=num_tests) as pbar:
        for bins, hierarchy, hist_dims in itertools.product(bins_list, hierarchy_list, hist_dims_list):
            if (hist_dims == 2 and (bins > 64 or hierarchy > 5)) or (hist_dims == 3 and (bins > 32 or hierarchy > 4)):
                pbar.set_postfix({
            ...
            db.change_hist(bins, hierarchy, hist_dims)
            for distance in distances_list:
                results = [[] for _ in k_list]
                for idx, query in enumerate(query_list):
                    start_time = time.time()
                    hist = histograms.gen_hist_hier(query, bins, hierarchy, hist_dims)
                ...
                if val:
                    ...
                    for idx, (result, k) in enumerate(zip(results, k_list)):
                        ...
                        if mapk > best_mapk[idx]:
                            best_config[idx] = [color_space, preprocess, bins, hierarchy, hist_dims, distance]
                            best_result[idx] = result
                            best_mapk[idx] = mapk
                    
                    grid_search_df.loc[len(grid_search_df)] = [color_space, preprocess, bins, hierarchy, hist_dims, distance, *mapk_list]
                    pbar.set_postfix({
                        "color_space": color_space,
                        "preprocess": preprocess,
                        "bins": bins,
    ...
    if val:
        ...
        grid_search_df.to_csv('results/block_results.csv')
